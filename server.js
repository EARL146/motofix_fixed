...
const testDatabase = async () => {
  try {
    ...
    // migration: add Google account ID for Google-only sign-in
    try {
    ...
    // migration: add TikTok account ID to users for customer-only TikTok Login Kit
    try {
      await conn.query("ALTER TABLE users ADD COLUMN tiktok_open_id VARCHAR(255) NULL UNIQUE");
      console.log('✅ tiktok_open_id column added to users');
    } catch(e) { /* already exists */ }

    // migration: keep TikTok OAuth tokens server-side only
    try {
      await conn.query(`
      ...
      `);
      console.log('✅ tiktok_accounts table ready');
    } catch(e) { console.warn('TikTok migration warning:', e.message); }
...
// Strict admin authorization — every /api/admin request must be authenticated
// as the one configured admin account. Frontend checks are not trusted.
const requireAdminAccess = async (req, res, next) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    if (!token) return res.status(403).json({ success: false, message: 'Admin authentication required.' });
    ...
    if (!decoded?.id || !decoded?.timestamp || (Date.now() - Number(decoded.timestamp)) > (24 * 60 * 60 * 1000)) {
      return res.status(403).json({ success: false, message: 'Admin session expired.' });
    }
    ...
    try {
      ...
      if (!user || user.role !== 'admin' || !adminEmails.includes(user.email.toLowerCase().trim()) || !adminEmails.includes((decoded.email || '').toLowerCase().trim())) {
...
app.use('/api/admin', requireAdminAccess, (req, res, next) => {
  req.upload = upload;
  next();
}, adminRoutes);
